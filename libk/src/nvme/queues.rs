...
impl SubmissionQueueEntry {
    pub(crate) fn new(
        ...
        entry.prps[0] = if let Some(prp) = dptr[0] { prp } else { 0 };
        entry.prps[1] = if let Some(prp) = dptr[1] { prp } else { 0 };
        (0..6).for_each(|idx| {
            if let Some(arg) = operands[idx] {
...
impl Status {
    ...
    pub fn to_string(&self) -> String<U128> {
        ...
        let msgstring = match self.sct {
            ...
            StatusCodeType::VendorSpecific => "Vendor specific",
            StatusCodeType::Other(_) => "Other",
        };
        match msg.push_str(msgstring) {
            Ok(()) => msg,
            Err(()) => panic!("Cannot stringify NVMe error message: string is too large"),
        }
    }
}
