...
use crate::{
    ...
    memory::{allocate_phys_range, free_range, get_free_addr},
    pci::PCIDevice,
};
...
impl NVMeController {
    ...
    pub unsafe fn new(device: PCIDevice) -> Option<Self> {
        ...
        let buf_loc = get_free_addr(16384);
        if !allocate_phys_range(buf_loc, buf_loc + 16384, false) {
            error!("Cannot allocate IO buffer!");
            return None;
        }
    ...
    #[inline]
    pub fn write_intms(&self, val: u32) {
    ...
    pub fn init(&mut self) {
        ...
        'nvme_init: loop {
            ...
            let mut asqaddr = get_free_addr(asqsize);
            asqaddr.set_bits(0..12, 0);
            let mut acqaddr = get_free_addr(acqsize);
        ...
        data.iter_mut().enumerate().for_each(|(i, b)| {
            debug!("Reading idx {}", i);
            let mem: VolBlock<u8, 4096> = unsafe { VolBlock::new(self.resp_buffer_addr as usize) };
            *b = mem.index(i).read();
            mem.index(i).write(0);
        });
